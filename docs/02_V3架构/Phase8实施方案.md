...
`/api/query/preview` 继续用于 SQL 编辑、字段识别和有限样本，响应必须明确是否截断。正式组件运行不能把最多 200 行的预览结果交给浏览器再计算业务聚合。

P8.4 的运行接口应接收由已保存数据集和组件配置生成的结构化查询计划，在服务端完成参数条件、分组、聚合、排序和 limit。客户端不能提交原始 SQL、任意字段名或任意运算符；服务端只接受数据集字段元数据和受限聚合枚举中的引用。

前端 `queryResult.ts` 可继续承担 mock、历史兼容和展示形态转换，但服务端数据集的正式 SUM、COUNT、TOP、百分比等结果必须来自完整数据上的服务端计算。
...
- 预览接口与正式运行接口分离，正式接口不复用截断样本作为业务结果。
- 正式接口依据受限查询计划在服务端完成分组、聚合、排序和 limit。
- 覆盖必填、空值、多选、日期范围、未知参数和危险输入。
- 使用超过 200 行的固定数据验证 SUM、COUNT、TOP 和百分比与校验 SQL 一致。
...
11. 正式组件查询不基于截断样本做浏览器聚合，大数据集结果与校验 SQL 一致。
...
| 预览截断导致正式指标错误 | 预览与运行分离，正式查询在服务端完整数据上聚合并做对账测试 |
