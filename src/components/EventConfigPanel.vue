...
import type { DashboardComponentV3, EventBindingV3, EventConditionV3, EventNameV3, ValueExpressionV3 } from '../models/dashboard-v3.ts'
...
<template>
  <div class="event-panel-backdrop" role="presentation" @click.self="requestClose">
    <aside class="event-config-panel" aria-label="事件配置">
      ...
      <p v-if="!authorableEvents.length" class="readonly-note">dialog 既有事件仅供只读查看</p>
      ...
      <div v-if="draft" class="event-editor" :class="{ readonly: readOnly }">
        ...
        <section><div class="section-title"><b>动作（按顺序）</b><span><button type="button" :disabled="readOnly" @click="addAction('setParameter')">+ SetParameter</button><button type="button" :disabled="readOnly" @click="addAction('refresh')">+ Refresh</button></span></div>
