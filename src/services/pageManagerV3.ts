...
function rewriteEvents(
  ...
  return events.map((event) => ({
    ...clone(event),
    id: idFactory('event', event.id),
    actions: event.actions.map((action) => {
      const copied = { ...clone(action), id: idFactory('action', action.id) }
      if (copied.type !== 'refresh') return copied
      if (copied.target.kind === 'components') {
        copied.target.componentIds = copied.target.componentIds.map((id) => componentIds.get(id) ?? id)
      } else if (copied.target.pageId === sourcePageId) {
        copied.target.pageId = targetPageId
      }
      return copied
    }),
  }))
...
export function createPageV3(
  ...
  if (options.type !== undefined && options.type !== 'standard') throw new Error('页面命令只允许创建 standard 页面')
...
export function copyPageV3(
  ...
  if (source.type !== 'standard') throw new Error('页面命令只允许复制 standard 页面')
  const name = options.name.trim()
  const code = options.code.trim()
  if (!name) throw new Error('页面名称不能为空')
  if (!/^[a-z][a-z0-9_]*$/.test(code)) throw new Error('页面编码格式无效')
  ...
  const page: DashboardPageV3 = {
    ...
    components: source.components.map((component) => ({
      ...
      ...(component.events ? { events: rewriteEvents(component.events, nextId, componentIds, source.id, pageId) } : {}),
    })),
    pageEvents: rewriteEvents(source.pageEvents, nextId, componentIds, source.id, pageId),
  }
  return { application: { ...clone(application), pages: [...clone(application.pages), page] }, pageId }
}
